...
def make_component_tabs(name, title, icon, cols, rows):
...
asistencia = make_component_tabs(
...
<td className="py-4 px-4 text-center"><span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase border tracking-wide ${item.estado === 'P' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>{item.estado === 'P' ? 'Presente' : item.estado === 'F' ? 'Falta' : item.estado === 'T' ? 'Tardanza' : item.estado}</span></td>
<td className="py-4 px-4 text-slate-500 italic">{item.observacion || '-'}</td>"""
)

pagos = make_component_tabs(
    "PagosPadre", "Estado de Cuenta", "💳",
    """<th className="py-4 px-4 font-bold text-slate-700">Concepto</th>
<th className="py-4 px-4 font-bold text-slate-700">Fecha</th>
<th className="py-4 px-4 font-bold text-slate-700 border-l border-slate-200 text-center bg-slate-100">Monto</th>""",
    """<td className="py-4 px-4 font-medium text-slate-800">{item.concepto}</td>
<td className="py-4 px-4 text-slate-600">{item.fecha}</td>
<td className="py-4 px-4 font-extrabold text-teal-800 text-center border-l border-slate-100 bg-teal-50/30">S/ {item.monto}</td>"""
)

conducta = make_component_tabs(
...
<td className="py-4 px-4 text-center"><span className={`px-4 py-1.5 rounded-full text-xs font-bold border ${(item.tipo || '').toLowerCase().includes('positiva') ? 'bg-blue-50 text-blue-700 border-blue-200' : (item.tipo || '').toLowerCase().includes('grave') ? 'bg-red-50 text-red-700 border-red-200' : 'bg-orange-50 text-orange-700 border-orange-200'}`}>{item.tipo}</span></td>
...
print("COMPONENTS REWRITTEN CON PESTANAS (TABS)")
